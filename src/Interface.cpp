...
Interface::Interface(const Dictionary& dictionary,
        std::istream& input_stream, std::ostream& output_stream, int fails_limit) :
        input_stream_(input_stream), output_stream_(output_stream),
        hidden_word_(dictionary.get_random_word()), is_finished_(false),
...
void Interface::process_char() {
    ...
    if (code == HiddenWord::OpenResultCode::SUCCESS) {
        ...
        --fails_limit_;
        if (fails_limit_ >= 0) {
            output_stream_ << "Fail! you have " << fails_limit_ << " fail chances" << std::endl;
        } else {
...
bool Interface::is_finished() const {
    return is_finished_;
}
